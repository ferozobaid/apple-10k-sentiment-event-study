/*****************************************************************************
 *  MODULE 06b — EVENT STUDY (reliable parameter extraction)
 *  ---------------------------------------------------------------------------
 *  Fix: use ODS OUTPUT instead of outest= to capture Market Model parameters
 *****************************************************************************/

libname proj "~/apple_sentiment_project";
title "MODULE 06b: Event Study (Fixed)";
...
/*===========================================================================*/
/*  6b.1 Assign trading-day index to filing dates                            */
/*===========================================================================*/
proc sql;
...
/*===========================================================================*/
/*  6b.3 Estimate Market Model — use ODS OUTPUT for reliable column names    */
/*===========================================================================*/
ods listing close;
ods output ParameterEstimates=work.pe;

proc reg data=work.event_panel;
...
/* Reshape: one row per fyear with alpha and beta as columns */
data work.params_clean;
    retain fyear alpha beta;
    set work.pe;
    by fyear;
    if first.fyear then do; alpha = .; beta = .; end;
    if upcase(Variable) = "INTERCEPT" then alpha = Estimate;
    else beta = Estimate;
...
proc print data=work.params_clean;
    ...
    format alpha 10.6 beta 6.3;
run;

/*===========================================================================*/
/*  6b.4 Compute abnormal returns                                            */
/*===========================================================================*/
proc sql;
...
/*===========================================================================*/
/*  6b.5 Cumulative Abnormal Returns (CAR) per filing                        */
/*===========================================================================*/
proc sql;
...
/*===========================================================================*/
/*  6b.7 AAR and CAAR by event-day                                           */
/*===========================================================================*/
...
proc print data=proj.apple_aar;
    title2 "AAR and CAAR by Event-Day";
run;
...
title3 "N=15 filings | Event Day = 0 | Market Model";
...
title2 "CAR[0,+3] by Individual Filing";
